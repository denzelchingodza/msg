...
export function PauseMenu({ onResume, onRestart, onSettings }: {
  onResume: () => void; onRestart: () => void; onSettings: () => void;
}) {
  return (
...
export function DailyPanel({ makes, goal, reward, claimed, onClaim, onClose }: {
  makes: number; goal: number; reward: number; claimed: boolean; onClaim: () => void; onClose: () => void;
}) {
  const done = makes >= goal;
  return (
    <GlassModal kicker="Daily challenge" title={`Make ${goal} threes`} onClose={onClose}>
